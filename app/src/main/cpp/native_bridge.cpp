...
//
// EmuCoreA libretro frontend for the vendored PPSSPP core.
//
...
#define LOG_TAG "EmuCoreA"
#define CORE_LOG_TAG "EmuCoreA-Core"
#ifdef NDEBUG
#define LOGI(...) ((void)0)
#define LOGW(...) ((void)0)
#define LOGE(...) ((void)0)
...
struct FrontendState {
    ...
    std::string save_dir;
    // The unmodified frontend save directory. PPSSPP derives its memstick
...
bool LoadCoreLocked() {
    ...
    void* handle = dlopen("ppsspp_libretro_android.so", RTLD_NOW | RTLD_LOCAL);
    if (handle == nullptr) {
        ...
        if (!library_dir.empty()) {
            const std::string path = library_dir + "/ppsspp_libretro_android.so";
            handle = dlopen(path.c_str(), RTLD_NOW | RTLD_LOCAL);
        }
    }
    if (handle == nullptr) {
        LOGE("Unable to load PPSSPP libretro core: %s", dlerror());
        return false;
    }
    ...
    if (g_core.set_environment == nullptr || g_core.init == nullptr || g_core.load_game == nullptr ||
        ...
        LOGE("PPSSPP libretro core is missing required symbols");
        dlclose(handle);
        return false;
    }

    g_core.handle = handle;
    LOGI("PPSSPP libretro core loaded");
    return true;
}
...
void RegisterCoreOptionsV2(const retro_core_options_v2* v2) {
    ...
    LOGI("Registered %zu PPSSPP core options", g_frontend.registered_options.size());
}
...
struct AudioOutput {
    ...
    std::atomic<int64_t> sourced_frames{0};
...
aaudio_data_callback_result_t AudioDataCallback(AAudioStream*, void* user_data, void* audio_data, int32_t num_frames) {
    ...
    // The ring is filled from the core's 44.1 kHz mixer; the stream may run at
    // the device rate, so the converter consumes core frames at a matching
...

    {
        ...
        // Always fills every frame: a shortfall is padded with the last sample
        // (like PPSSPP) instead of muting and resetting the converter.
        sourced = output->resampler.Read(g_frontend.audio_ring.data(), capacity,
    ...
    output->sourced_frames.fetch_add(static_cast<int64_t>(sourced));
    output->queued_frames.store(static_cast<int64_t>(sourced));
    output->silence_frames.fetch_add(static_cast<int64_t>(num_frames) - static_cast<int64_t>(sourced));
...
void PresentHardwareFrame(GLsizei src_width, GLsizei src_height) {
    ...
    const double aspect = src_height > 0 ? static_cast<double>(src_width) / src_height
                                         : g_frontend.aspect_ratio;
    ...
    if (emucorea::shader_chain_present::Present(
        ...
        emucorea::shader_effect::Present(
...
void DestroyHardwareContext() {
    ...
    emucorea::shader_effect::Destroy();
    DestroyFramebuffer();
    if (g_gl.context != EGL_NO_CONTEXT) {
        eglMakeCurrent(g_gl.display, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT);
    }
...
// Input.
// ---------------------------------------------------------------------------
// The Kotlin layer keeps the EmuCoreR active-low pad bitmask convention:
...
// com.sbro.emucorea.core.NativeCoreBridge.
// ---------------------------------------------------------------------------
extern "C" {
...
size_t EmuCoreAGetMemorySize(unsigned id) {
    return g_core.get_memory_size != nullptr ? g_core.get_memory_size(id) : 0;
}

JNIEXPORT void JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_nativeInit(JNIEnv* env, jobject, jstring system_dir,
...
Java_com_sbro_emucorea_core_NativeCoreBridge_createSession(JNIEnv*, jobject) {
...
JNIEXPORT void JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_destroySession(JNIEnv*, jobject, jlong handle) {
    if (handle == 0) return;
    std::lock_guard<std::mutex> lock(g_frontend.core_mutex);
    ...
    EmuCoreAAchievementsOnSessionEnd();
    // The frame worker has already joined. Rebind its EGL context on this
    // teardown thread before PPSSPP releases GPU resources in context_destroy
...
Java_com_sbro_emucorea_core_NativeCoreBridge_reset(JNIEnv*, jobject, jlong handle) {
...
Java_com_sbro_emucorea_core_NativeCoreBridge_runFrame(JNIEnv* env, jobject, jlong handle) {
    ...
    // PPSSPP updates its AV geometry when internal resolution changes. The
...
JNIEXPORT void JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_setTimeControl(JNIEnv*, jobject, jint mode) {
...
Java_com_sbro_emucorea_core_NativeCoreBridge_ensureHardwareContext(JNIEnv*, jobject) {
    return EnsureHardwareContext() ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT void JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_setDisplayAspectRatio(JNIEnv*, jobject, jint mode) {
    g_frontend.display_aspect_mode.store(mode >= 0 && mode <= 4 ? mode : 1);
}

JNIEXPORT jint JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_setSurface(JNIEnv* env, jobject, jlong handle,
...
JNIEXPORT void JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_nativeSetOption(JNIEnv* env, jobject, jstring key,
                                                             jstring value) {
...
Java_com_sbro_emucorea_core_NativeCoreBridge_nativeGetOption(JNIEnv* env, jobject, jstring key) {
...
JNIEXPORT void JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_nativeSetShaderEffect(JNIEnv*, jobject, jint effect) {
    emucorea::shader_effect::Set(effect);
    vulkan::SetShaderEffect(effect);
}

JNIEXPORT void JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_nativeSetShaderPreset(JNIEnv* env, jobject,
                                                                     jstring path, jboolean enabled) {
    emucorer::shader_chain::SetPreset(ToString(env, path), enabled == JNI_TRUE);
}

JNIEXPORT jint JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_loadBios(JNIEnv*, jobject, jlong handle, jstring) {
    // PPSSPP does not need a BIOS image; firmware assets are handled by the
    // frontend data directory.
    return handle == 0 ? -1 : 0;
}

JNIEXPORT jint JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_loadBiosOnly(JNIEnv*, jobject, jlong handle) {
    return handle == 0 ? -1 : -3;
}

JNIEXPORT jint JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_loadDisc(JNIEnv* env, jobject, jlong handle, jstring path) {
...
JNIEXPORT void JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_setPadButtons(JNIEnv*, jobject, jlong handle,
...
JNIEXPORT void JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_setPadAnalog(JNIEnv*, jobject, jlong handle, jint port,
...
JNIEXPORT void JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_setPadAnalogMode(JNIEnv*, jobject, jlong handle, jint port,
                                                              jboolean enabled) {
    if (handle == 0 || port < 0 || port > 1) return;
...
JNIEXPORT jint JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_getPadState(JNIEnv*, jobject, jlong handle, jint port) {
...
JNIEXPORT jint JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_saveState(JNIEnv* env, jobject, jlong handle, jstring path) {
...
JNIEXPORT jint JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_loadState(JNIEnv* env, jobject, jlong handle, jstring path) {
...
JNIEXPORT jint JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_createMemoryCard(JNIEnv*, jobject, jstring) {
...
JNIEXPORT void JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_setMemoryCardPath(JNIEnv*, jobject, jint, jstring) {}

JNIEXPORT void JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_setTextureReplacementsPathOverride(JNIEnv* env, jobject,
                                                                                 jstring path) {
    ...
    LOGI("Texture replacement root=%s, PPSSPP memstick=%s", texture_directory.c_str(),
         g_frontend.save_dir.c_str());
}

JNIEXPORT void JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_loadCheats(JNIEnv* env, jobject, jstring source_path) {
...
JNIEXPORT void JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_clearCheats(JNIEnv*, jobject) {
    if (g_core.cheat_reset != nullptr) g_core.cheat_reset();
}

JNIEXPORT jstring JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_getSystemInfo(JNIEnv* env, jobject) {
    if (!LoadCoreLocked() || g_core.get_system_info == nullptr) {
        return env->NewStringUTF("PPSSPP");
    }
    retro_system_info info{};
    g_core.get_system_info(&info);
    std::string name = info.library_name != nullptr ? info.library_name : "PPSSPP";
...
JNIEXPORT jstring JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_getDiagnostics(JNIEnv* env, jobject) {
    char buffer[256];
    std::snprintf(buffer, sizeof(buffer),
                  "{\"core\":\"PPSSPP\",\"initialized\":%d,\"game\":%d,\"renderer\":%d,\"w\":%u,\"h\":%u}",
...
JNIEXPORT jint JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_apiVersion(JNIEnv* env, jobject) {
    if (!LoadCoreLocked()) {
        jclass exception_class = env->FindClass("java/lang/IllegalStateException");
        if (exception_class != nullptr) {
            env->ThrowNew(exception_class, "PPSSPP libretro core could not be loaded");
        }
        return 0;
    }
...
Java_com_sbro_emucorea_core_NativeCoreBridge_getDisplayRect(JNIEnv* env, jobject, jlong handle) {
...
Java_com_sbro_emucorea_core_NativeCoreBridge_getPresentRect(JNIEnv* env, jobject) {
...
Java_com_sbro_emucorea_core_NativeCoreBridge_getFrameRate(JNIEnv*, jobject, jlong handle) {
...
Java_com_sbro_emucorea_core_NativeCoreBridge_getAvInfo(JNIEnv* env, jobject, jlong handle) {
    retro_system_av_info info{};
...
JNIEXPORT jboolean JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_hasDiscMedia(JNIEnv*, jobject, jlong handle) {
...
JNIEXPORT jstring JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_getDiscMetadataFd(JNIEnv*, jobject, jint, jlong, jlong) {
    return nullptr;
}

JNIEXPORT void JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_setAudioOutputLatencyMs(JNIEnv*, jobject, jint milliseconds) {
    g_frontend.audio_output_latency_ms.store(std::clamp(static_cast<int>(milliseconds), 1, 500));
}

JNIEXPORT void JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_setAudioLowLatency(JNIEnv*, jobject, jboolean enabled) {
    g_frontend.audio_low_latency.store(enabled == JNI_TRUE);
}

JNIEXPORT void JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_setFrameSkip(JNIEnv*, jobject, jint frames) {
...
JNIEXPORT void JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_setAudioGain(JNIEnv*, jobject, jfloat gain) {
...
JNIEXPORT jlong JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_createAudioOutput(JNIEnv*, jobject) {
...
JNIEXPORT jint JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_startAudioOutput(JNIEnv*, jobject, jlong handle) {
    if (handle == 0) return -1;
    auto* output = reinterpret_cast<AudioOutput*>(handle);
...
JNIEXPORT jint JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_pauseAudioOutput(JNIEnv*, jobject, jlong handle) {
    if (handle == 0) return -1;
    auto* output = reinterpret_cast<AudioOutput*>(handle);
    if (output->stream == nullptr) return -2;
...
JNIEXPORT jint JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_flushAudioOutput(JNIEnv*, jobject, jlong handle) {
    if (handle == 0) return -1;
    auto* output = reinterpret_cast<AudioOutput*>(handle);
...
JNIEXPORT jint JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_audioOutputBufferedFrames(JNIEnv*, jobject, jlong handle) {
    if (handle == 0) return -1;
    auto* output = reinterpret_cast<AudioOutput*>(handle);
...
JNIEXPORT jint JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_audioOutputPacingHighWaterFrames(JNIEnv*, jobject, jlong handle) {
    if (handle == 0) return -1;
    auto* output = reinterpret_cast<AudioOutput*>(handle);
...
JNIEXPORT jlongArray JNICALL
Java_com_sbro_emucorea_core_NativeCoreBridge_audioOutputStats(JNIEnv* env, jobject, jlong handle) {
    ...
    if (handle != 0) {
        ...
        values[3] = output->burst_frames;
        values[4] = output->queued_frames.load();
        values[5] = output->sourced_frames.load();
