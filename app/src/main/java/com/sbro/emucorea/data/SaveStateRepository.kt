...
class SaveStateRepository(private val context: Context) {
    ...
    fun analyzeImport(
        ...
        try {
            ...
            val directVersion = readEmuCoreASaveStateVersion(stagedFile.inputStream())
            ...
            val importCandidate: (SaveStateImportCandidate, ZipFile?) -> Unit = { candidate, zip ->
                    ...
                    if (target.exists()) {
                        ...
                        val success = runCatching {
                            ...
                            check(readEmuCoreASaveStateVersion(temporary.inputStream()) == CURRENT_SAVE_STATE_VERSION) {
                                "Invalid EmuCoreA save state"
                            }
    ...
    private fun collectArchiveCandidates(zip: ZipFile): List<RawImportCandidate> {
        return zip.entries().asSequence()
            ...
            .map { entry ->
                val version = zip.getInputStream(entry).use(::readEmuCoreASaveStateVersion)
                RawImportCandidate(
...
private fun readEmuCoreASaveStateVersion(input: InputStream): Int? {
