...
#ifdef __ANDROID__
	// Android's ART signal chain can install or update its SIGSEGV handling
	// while a game is running. Restoring the saved process-wide action on each
	// game exit can discard that chain, so a later ART implicit null check
	// terminates the whole app. Keep PPSSPP's handler for the process lifetime;
	// it rejects non-PPSSPP faults and forwards them to the prior handler.
	return;
#endif
