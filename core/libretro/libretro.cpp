...
static void check_variables(CoreParameter &coreParam)
{
   ...
   var.key = "ppsspp_memstick_size";
   if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value) {
      const int size = atoi(var.value);
      if (size >= 1 && size <= 128 && (size & (size - 1)) == 0)
         g_Config.iMemStickSizeGB = size;
   }
   ...
   if (g_Config.iInternalResolution != iInternalResolution_prev && backend != RETRO_HW_CONTEXT_NONE)
   {
      ...
      if (gpu)
      {
         ...
         {
            // Vulkan keeps its context across live resolution changes. Resize the
            // game's render targets as well as the presentation images; otherwise
            // only a later renderer restart applies the selected resolution.
...
void retro_deinit(void)
{
   if (rumble_interface.set_rumble_state) {
      rumble_interface.set_rumble_state(0, RETRO_RUMBLE_STRONG, 0);
      rumble_interface.set_rumble_state(0, RETRO_RUMBLE_WEAK, 0);
   }
   rumble_interface = {};
...
void retro_run(void) {
   ...
   if (rumble_interface.set_rumble_state) {
      ...
      rumble_interface.set_rumble_state(0, RETRO_RUMBLE_WEAK,
         sceCtrlGetRightVibration());
   }
}
...
// standard libretro cheat_set API only accepts one code at a time and does
// not preserve the frontend's named multi-line blocks.
extern "C" __attribute__((visibility("default"))) void emucorea_cheat_reload(void) {
   g_Config.bReloadCheats = true;
}
