...
static std::atomic<u16> leftVibration{0};
static std::atomic<u16> rightVibration{0};
...
void __CtrlVblank() {
	...
	leftVibration.store(static_cast<u16>(leftVibration.load() * (float)vibrationLeftDropout / 256.0f));
	rightVibration.store(static_cast<u16>(rightVibration.load() * (float)vibrationRightDropout / 256.0f));
...
u16 sceCtrlGetRightVibration() {
	return hleNoLog(rightVibration.load());
}

u16 sceCtrlGetLeftVibration() {
	return hleNoLog(leftVibration.load());
}

namespace SceCtrl {
	void SetRightVibration(u16 rVibration) {
		rightVibration.store(rVibration);
	}
	void SetLeftVibration(u16 lVibration) {
		leftVibration.store(lVibration);
	}
